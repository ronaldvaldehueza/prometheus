...
interface SearchProjectProps extends RouteComponentProps<MatchParams> { }

const SearchProject: React.FC<SearchProjectProps> = (props) => {
  const [projects, setProjects] = useState<ProjectData[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    searchProject(props.match.params.input);
  }, [props.match.params.input]);
  ...
  const searchProject = (title: string) => {
    setLoading(true);
    projectHTTPService.searchProject(title).then(response => {
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h4><i className="menu-icon fa fa-search"></i> Search Results</h4>
      </div>
